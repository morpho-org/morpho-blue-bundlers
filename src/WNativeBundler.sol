...
abstract contract WNativeBundler is BaseBundler {
    ...
    /// @dev Use `TransferBundler.erc20Transfer` to transfer the wrapped native tokens to some `receiver`.
    ...
    /// @notice Warning: should only be called via the bundler's `multicall` function.
    /// @dev Use `TransferBundler.nativeTransfer` to transfer the unwrapped native tokens to some `receiver`.
